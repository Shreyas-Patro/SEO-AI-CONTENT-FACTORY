...
def _load_agents():
    """
    Import agent functions only. No persistent DB connection is kept open.
    Holding a connection inside st.cache_resource causes 'database is locked'
    on Windows when agents try to write during a pipeline run.
    """
    try:
        ...
        # init_db opens and immediately closes its own connection — safe
        init_db()
        return {
...
# ── PATCH SQLITE BUSY TIMEOUT ──────────────────────────────────────────────
# Windows file locking is stricter than Linux. busy_timeout=10000 tells
# SQLite to retry for 10s before raising "database is locked" instead of
# failing immediately. This is the fix for Content Architect db lock errors.
def _patch_sqlite():
    try:
        import sqlite3
