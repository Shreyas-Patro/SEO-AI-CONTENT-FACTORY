...
def _render_live_log(run_id):
    ...
    try:
        ...
        return
    st.code(
        text[-7000:] if len(text) > 7000 else text or "(no output yet)",
        language=None,
    )
...
def render(m):
    ...
    with atabs[1]:
        _render_agent_detail(rid, "competitor_spy", m)
    with atabs[2]:
        _render_agent_detail(rid, "keyword_mapper", m)
    with atabs[3]:
    ...
    with atabs[4]:
        _render_agent_detail(rid, "content_architect", m)
    with atabs[5]:
        _render_agent_detail(rid, "faq_architect", m)
    with atabs[6]:
        _render_agent_detail(rid, "research_prompt_generator", m)
    with atabs[7]:
        _render_agent_detail(rid, "lead_writer", m)
    with atabs[8]:
        _render_agent_detail(rid, "fact_verifier", m)
    with atabs[9]:
        _render_agent_detail(rid, "brand_auditor", m)
    with atabs[10]:
